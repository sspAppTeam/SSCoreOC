...
#pragma mark - Request
-(RACSignal *) enqueueRequest:(MHHTTPRequest *) request resultClass:(Class /*subclass of SSBaseModel*/) resultClass{
    /// request 必须的有值
...
- (RACSignal *)enqueueUploadRequest:(MHHTTPRequest *)request resultClass:(Class)resultClass fileDatas:(NSArray<NSData *> *)fileDatas name:(NSString *)name mimeType:(NSString *)mimeType{
    ...
    return [[[self enqueueUploadRequestWithPath:request.urlParameters.path parameters:request.urlParameters.parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (int i = 0; i< count; i++) {
            ...
            [formData appendPartWithFileData:fileData name:name fileName:fileName mimeType:StringIsNotEmpty(mimeType)?mimeType:@"application/octet-stream"];
        }
    }]
             reduceEach:^RACStream *(NSURLResponse *response, NSDictionary * responseObject){
                 @strongify(self);
...
- (RACSignal *)parsedResponseOfClass:(Class)resultClass fromJSON:(NSDictionary *)responseObject {
    /// 必须是SSBaseModel的子类 且 最外层responseObject必须是字典
    NSParameterAssert((resultClass == nil || [resultClass isSubclassOfClass:SSBaseModel.class]));
    ...
    return [RACSignal createSignal:^ id (id<RACSubscriber> subscriber) {
        ...
        void (^parseJSONDictionary)(NSDictionary *) = ^(NSDictionary *JSONDictionary) {
            ...
            if ([JSONArray isKindOfClass:[NSArray class]]) {
                /// 字典数组 转对应的模型
                 NSArray *parsedObjects=[resultClass.class mj_objectArrayWithKeyValuesArray:JSONArray];
                /// 这里还需要解析是否是SSBaseModel的子类
                for (id parsedObject in parsedObjects) {
                    /// 确保解析出来的类 也是 SSBaseModel
                    NSAssert([parsedObject isKindOfClass:SSBaseModel.class], @"Parsed model object is not an SSBaseModel: %@", parsedObject);
                }
                [subscriber sendNext:parsedObjects];
                
            }else{
                /// 字典转模型
                SSBaseModel *parsedObject = [resultClass.class mj_objectWithKeyValues:JSONDictionary];
                if (parsedObject == nil) {
                ...
                NSAssert([parsedObject isKindOfClass:SSBaseModel.class], @"Parsed model object is not an BaseModel: %@", parsedObject);
                /// 发送数据
...
-(NSString *)_signWithParameters:(NSDictionary *) parameters {
    ...
    for (id key in sortedKeys) {
        /// value 为 empty 跳过
        if(ObjectIsNil(parameters[key])) continue;
        NSString * value = [parameters[key] mh_stringValueExtension];
        if (ObjectIsNil(value)||!StringIsNotEmpty(value)) continue;
